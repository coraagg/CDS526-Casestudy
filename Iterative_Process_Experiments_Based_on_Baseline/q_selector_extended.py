...
Description: Inherits from QLearningSelector and overrides the get_state() method.
             Replaces the Euclidean distance in decision space with crowding distance
             (computed from the objective space) as the population diversity feature.
             The method also includes safety checks for NaN/Inf values and falls back to
             Euclidean distance when crowding distance is unavailable.
"""
...
class QLearningSelectorExtended(QLearningSelector):
    def get_state(self, algorithm, problem):
        ...
        # Diversity with crowding distance
        if hasattr(algorithm, 'crowding') and algorithm.crowding is not None:
            ...
            # Downgrading: Use Euclidean distance (the same as the original)
            pop = algorithm.pop
            if len(pop) > 1:
                distances = []
